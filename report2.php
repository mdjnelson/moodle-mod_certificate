...
if (!$users = certificate_get_noissues($certificate->id, $DB->sql_fullname(), $groupmode, $cm, $page, $perpage)) {
    echo $OUTPUT->header();
...
if ($download == "ods") {
    ...
    $myxls->write_string(0, 4, $stremail);
...
$usercount = count(certificate_get_noissues($certificate->id, $DB->sql_fullname(), $groupmode, $cm));

// Create the table for the users
$table = new html_table();
$table->width = "95%";
$table->tablealign = "center";
$table->head  = array($strto, $stremail, $strgrade, $strcode);
$table->align = array("left", "left", "center", "center");
foreach ($users as $user) {
    $name = $OUTPUT->user_picture($user) . fullname($user);
    $email = $user->email;
    $code = $user->code;
    $table->data[] = array ($name, $email, certificate_get_grade($certificate, $course, $user->id), $code);
}

// Create table to store buttons
$tablebutton = new html_table();
$tablebutton->attributes['class'] = 'downloadreport';
$btndownloadods = $OUTPUT->single_button(new moodle_url("report2.php", array('id'=>$cm->id, 'download'=>'ods')), get_string("downloadods"));
$btndownloadxls = $OUTPUT->single_button(new moodle_url("report2.php", array('id'=>$cm->id, 'download'=>'xls')), get_string("downloadexcel"));
$btndownloadtxt = $OUTPUT->single_button(new moodle_url("report2.php", array('id'=>$cm->id, 'download'=>'txt')), get_string("downloadtext"));
...
echo html_writer::tag('div', html_writer::table($tablebutton), array('style' => 'margin:auto; width:80%'));
echo $OUTPUT->footer($course);
